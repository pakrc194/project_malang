...
CREATE TABLE RESERVATION_DETAIL (
    ...
    FOREIGN KEY (resv_id) REFERENCES RESERVATION(resv_id) ON DELETE CASCADE,
...
-- 16. SEAT_STATUS (스케줄별 좌석 상태) - seat_status 컬럼명 변경 반영
CREATE TABLE SEAT_STATUS (
    status_id INT PRIMARY KEY AUTO_INCREMENT COMMENT '상태 ID',
    schedule_id INT NOT NULL COMMENT '스케줄 FK',
    seat_id INT NOT NULL COMMENT '좌석 FK',
    seat_status ENUM('Available', 'TempReserved', 'Reserved') NOT NULL COMMENT '좌석 상태', -- ⭐ 컬럼명 변경
